...
def parse_ports(ports_str: Optional[str]) -> List[int]:
    """
    Parse a port specification string like "22,80,443,8000-8010" into a sorted list of unique ints.
    If ports_str is None, returns DEFAULT_PORTS.
    """
    if not ports_str:
    ...
    try:
        ...
        for p in parts:
            if "-" in p:
                ...
                if lo_i > hi_i:
                    raise ValueError("Invalid port range: {}".format(p))
        ...
        raise argparse.ArgumentTypeError("Invalid ports specification: {}".format(e))

def worker(fin_event: threading.Event,
           host: str,
    ...
    while not fin_event.is_set():
        ...
        try:
            ...
            result_queue.put((host, port, status, err_msg))
            port_queue.task_done()


def printer(fin_event: threading.Event,
            result_queue: queue.Queue,
    ...
    try:
        while True:
            try:
                host, port, status, err = result_queue.get(timeout=1)
            except queue.Empty:
                if fin_event.is_set() and result_queue.empty():
                    return
                continue
...
def main():
    parser = argparse.ArgumentParser(description="Multi-threaded TCP connect port scanner (Python 3).")
    parser.add_argument("host", help="Target IP address or hostname")
    parser.add_argument("--ports", "-p",
                        help="Comma-separated ports and ranges (e.g. 22,80,443,8000-8010). If omitted, the original PORTS list will be used.")
    parser.add_argument("--threads", "-t", type=int, default=DEFAULT_THREAD_COUNT,
                        help=f"Worker thread count (default {DEFAULT_THREAD_COUNT})")
    parser.add_argument("--timeout", type=float, default=DEFAULT_TIMEOUT,
                        help=f"Socket connect timeout seconds (default {DEFAULT_TIMEOUT})")
    ...
    try:
        ...
        parser.error(str(e))
    if not ports_list:
        parser.error("No ports parsed; check --ports input or the embedded DEFAULT_PORTS list.")
...
...         t = threading.Thread(target=worker, args=(fin, args.host, port_q, result_q, args.timeout), daemon=True)
...         t.start()
...         workers.append(t)
... 
...     printer_thread = threading.Thread(target=printer, args=(fin, result_q, args.output), daemon=True)
...
...         print("\n[!] Interrupted by user. Requesting shutdown...")
...         fin.set()
...     finally:
...         fin.set()
